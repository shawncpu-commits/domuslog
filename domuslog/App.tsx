  ...
  useEffect(() => {
    ...
    if (condoId.startsWith('DB_')) {
      ...
      if (localDataStr) {
        ...
        setTransactions((localData.transactions || []).filter((t: any) => t.date.startsWith(selectedYear)));
        ...
        setWaterReadings((localData.waterReadings || []).filter((r: any) => r.date.startsWith(selectedYear)));
  ...
  const syncToStorage = useCallback(async (field: string, newValue: any) => {
    ...
    if (condoId.startsWith('DB_')) {
      try {
        ...
        if (field === 'regulation') await supabase.from('condos').update({ regulation: newValue }).eq('id', condoId);
        else await supabase.from(tableMap[field]).upsert(newValue.map((i: any) => ({ ...i, condo_id: condoId, year: (field === 'transactions' || field === 'waterReadings') ? selectedYear : undefined })));
  ...
  const handleAccessAdmin = async (e: React.FormEvent) => {
    ...
    try {
      if (isAuthModeRegister) {
        const { error } = await (supabase.auth as any).signUp({ email: adminEmail, password: adminPassword });
        if (error) throw error;
        alert("Account creato. Accedi ora.");
        setIsAuthModeRegister(false);
      } else {
        const { error } = await (supabase.auth as any).signInWithPassword({ email: adminEmail, password: adminPassword });
        if (error) throw error;
